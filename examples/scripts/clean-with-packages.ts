...
export default lauf({
  description: 'Clean build artifacts using packages field (no project dependency needed)',
  args: {
    targets: z
      .string()
  ...
  // rimraf comes from workspace-level packages in lauf.config.ts
  // No need to add it as a project dependency!
  async run(ctx) {
    // Dynamic import works because workspace packages are in NODE_PATH
