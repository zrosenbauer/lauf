...
describe('run handler', () => {
  ...
  it('runs script successfully when found by name', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.any(Object), {
      configDir: '/workspace',
    });
  ...
  it('parses --key=value args', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ name: 'test' }), {
      configDir: '/workspace',
    });
  });
  ...
  it('parses --key value args', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ name: 'hello' }), {
      configDir: '/workspace',
    });
  });
  ...
  it('parses --flag as boolean true', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ verbose: true }), {
      configDir: '/workspace',
    });
  });
  ...
  it('coerces numeric string values to numbers', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ count: 42 }), {
      configDir: '/workspace',
    });
  });
  ...
  it('coerces empty string value as-is', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ label: '' }), {
      configDir: '/workspace',
    });
  });
  ...
  it('does not coerce hex strings to numbers', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ val: '0xff' }), {
      configDir: '/workspace',
    });
  });
  ...
  it('does not coerce scientific notation to numbers', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ val: '1e10' }), {
      configDir: '/workspace',
    });
  });
  ...
  it('sliceArgvAfter only searches after index 2', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({ count: 5 }), {
      configDir: '/workspace',
    });
  });
  ...
  it('returns empty args when script name is not found in argv', async () => {
    ...
    expect(runScript).toHaveBeenCalledWith(mockScript, expect.objectContaining({}), {
      configDir: '/workspace',
    });
    const passedArgs = vi.mocked(runScript).mock.calls[0][1] as Record<string, unknown>;
