...
describe('extractArgMeta', () => {
  it('extracts metadata from string arg', () => {
    const args = { name: z.string().describe('The name') };
    const meta = extractArgMeta(args);
  ...
  it('extracts metadata from boolean arg', () => {
    const args = { verbose: z.boolean().describe('Enable verbose') };
    const meta = extractArgMeta(args);
    expect(meta).toHaveLength(1);
    expect(meta[0]).toMatchObject({
      name: 'verbose',
  ...
  it('extracts metadata from number arg', () => {
    const args = { count: z.number().describe('Item count') };
    const meta = extractArgMeta(args);
    expect(meta).toHaveLength(1);
    expect(meta[0]).toMatchObject({
      name: 'count',
  ...
  it('detects required args', () => {
    const args = { name: z.string() };
    const meta = extractArgMeta(args);
    expect(meta[0]).toMatchObject({ required: true });
  });

  it('detects optional args with defaults', () => {
    const args = { verbose: z.boolean().default(false) };
    const meta = extractArgMeta(args);
    expect(meta[0]).toMatchObject({
      required: false,
      defaultValue: false,
  ...
  it('resolves nullable schema type via anyOf', () => {
    const args = { name: z.string().nullable().describe('Nullable name') };
    const meta = extractArgMeta(args);
    expect(meta).toHaveLength(1);
    expect(meta[0]).toMatchObject({
      name: 'name',
  ...
  it('resolves optional nullable schema type', () => {
    const args = { label: z.string().nullable().optional().describe('Optional label') };
    const meta = extractArgMeta(args);
    expect(meta).toHaveLength(1);
    expect(meta[0]).toMatchObject({
      name: 'label',
  ...
  it('returns string type when schema has no type, anyOf, or oneOf', () => {
    // z.any() produces a JSON Schema property with no type info: {}
    const args = { data: z.any() };
    const meta = extractArgMeta(args);
    expect(meta).toHaveLength(1);
...
describe('formatHelp', () => {
  ...
  it('shows no flags message when no args', () => {
    const result = formatHelp('my-pkg/script', 'Does stuff', []);
  ...
  it('shows default value for optional args', () => {
    const meta = [
  ...
  it('shows default value string for args with default values', () => {
    const meta = [
      {
        name: 'host',
        type: 'string',
...
describe('extractArgMeta with mocked toJSONSchema', () => {
  ...
  it('returns empty metadata when toJSONSchema returns undefined', async () => {
    toJSONSchemaOverride.value = null;
    const mod = await import('./help.ts');
    const { z: mockedZ } = await import('zod');
    const meta = mod.extractArgMeta({ name: mockedZ.string() });
